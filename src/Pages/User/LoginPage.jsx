...
export default function LoginPage() {
  ...
  async function postData(e) {
    ...
    let item = response.find(x => (x.username?.trim().toLocaleLowerCase() === data.username?.trim().toLocaleLowerCase() ||
      x.email?.trim().toLocaleLowerCase() === data.username?.trim().toLocaleLowerCase()) &&
      x.password.trim() === data.password.trim()
    )
                ...
                <form onSubmit={postData}>
                  <div className="row">
                    ...
                    <div className={`col-12 ${errorMessage.password ? '' : 'mb-3'}`}>
                      ...
                      <input type="password" name="password" onChange={getInputData} placeholder='Password' className={`form-control ${errorMessage.password ? 'border-danger' : 'border-primary'}`} />
                    </div>
